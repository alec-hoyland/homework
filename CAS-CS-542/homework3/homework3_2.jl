...
function autoencodercost(theta::Array{Float64,1}, visiblesize, hiddensize, data::Array{Float64,2}, grad::Vector{Float64};
        ...
        avghiddenactivity = mean(a2)
        sparsitycost = sum(ρ*log(ρ/avghiddenactivity) + (1 - ρ)*log((1-ρ)/(1-avghiddenactivity)))
        cost = sum((output - data).^2)/(2*m) + λ/2*sum(theta[1:2*hiddensize*visiblesize].^2) + β*sparsitycost

        if length(grad) > 0
            # Back-propagation for sparsified objective
            a3prime = a3.*(one(a3[1]).-a3)
            a2prime = a2.*(one(a2[1]).-a2)
            sparsityterm = β*(-ρ/avghiddenactivity + (1-ρ)/(1-avghiddenactivity))

            d3 = -(data - a3).* a3prime
            d2 = broadcast(+,W2'*d3,sparsityterm).*a2prime

            grad[1:visiblesize*hiddensize] = (d2*a1'/m + λ*W1)[:] #dW1
            grad[visiblesize*hiddensize+1:2*visiblesize*hiddensize] = (d3*a2'/m + λ*W2)[:] #dW2
            grad[2*visiblesize*hiddensize+1:2*hiddensize*visiblesize+hiddensize] = (sum(d2,dims=2)/m)[:]
            grad[2*hiddensize*visiblesize+hiddensize+1:end] = (sum(d3,dims=2)/m)[:]
        end
        return cost, grad
...
function testGradient()
        vars = npzread("images.npy")
        traindata = Array{Float64, 2}(undef, SAMPLE_WIDTH*SAMPLE_HEIGHT,100)
        sampleimages(vars,100)
        ...
        function justcost(x)
                c,g = autoencodercost(x,visible,hidden,traindata, ones(size(x)); λ=λ,ρ=ρ,β=β)
                return c
        end
        cost,grad = autoencodercost(theta,visible,hidden,traindata, ones(size(theta)); λ=λ,ρ=ρ,β=β)
...
using NLopt
export autoencode
function autoencode(data,hiddensize,visiblesize;λ=1e-4,β=3.0,rho=0.01,maxiter=600)
        theta::Vector{Float64} = initializeparameters(hiddensize,visiblesize)
        count = 0
        function objective(x,grad)
                count += 1
                println("iteration $count")
                return autoencodercost!(x,visiblesize,hiddensize,data, grad, λ=λ,ρ=rho,β=β)
        end
...
function main()
        vars = npzread("images.npy")
        traindata = sampleimages(vars,10000)
