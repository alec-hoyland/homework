mutable struct NeuralNetwork
    ...
    activation_nodes::Array{Array{Float64, 1}, 1}
    weights::Array{Array{Float64, 2}, 1}
    last_changes::Array{Array{Float64, 2}, 1}
...
function NeuralNetwork(structure::Array{Int64, 1}, disable_bias::Bool)
    ...
    NeuralNetwork(structure,
                  ...
                  (y::Float64) -> y*(1-y),
                  Array{Array{Float64, 1}, 1}(undef, len_struct),
                  Array{Array{Float64, 2}, 1}(undef, len_weights),
                  Array{Array{Float64, 2}, 1}(undef, len_weights),
                  Array{Array{Float64, 1}, 1}(undef, 1)
                  )
end
...
function init_weights(network::NeuralNetwork)
    for i in 1:length(network.weights)
        # arr = Array{Float64, 1}(undef, length(network.activation_nodes[i]), network.structure[i+1])
        arr = Array{Float64, 2}(undef, length(network.activation_nodes[i]), network.structure[i+1])
...
function calculate_output_deltas(network::NeuralNetwork, expected_values::Vector{Float64})
    ...
    output_deltas = Array{Float64, 1}(undef, length(err))
    for i=1:length(err)
...
function calculate_internal_deltas(network::NeuralNetwork)
    ...
    for layer_index=2:length(network.activation_nodes)-1
        layer_deltas = Array{Float64, 1}(undef, length(network.activation_nodes[layer_index]))
        ...
        push!(network.deltas, layer_deltas)
    end
end
